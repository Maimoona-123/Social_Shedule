...
const AIComposer = () => {
  ...
  const [scheuledPost, setScheduledPost] = useState("");
  const [scheuledTime, setScheduledTime] = useState("");
  const [scheuledDate, setScheduledDate] = useState("");
  ...
  return (
    <div className='max-w-4xl mx-auto space-y-12 pb-20 animate-in-fade duration-700'>
      ...
      <div className='space-y-6 text-center mt-20'>
        ...
        <div className='relative group mt-12'>
          <textarea className='w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900
           placeholder-slate-400 outline-none foucs:border-slate-400 transition resize-none h-40'
            placeholder='Share ur idea.. (e.g. A post about the launch of our new eco-freindly coffe beans)'
          ...
          <div className='absolute bottom-4 right-2.5 flex items-center gap-3 text-sm '>
            <button onClick={() => setGenerateImage(!generateImage)}
              ...
              <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors
                  duration-200 ease-in-out foucs:outline-none ${generateImage ? "bg-red-500" : "bg-slate-200"}`}>
      ...
      <div className='space-y-6 pt-12 border-t border-slate-100'>
        <div className='space-y-6 pt-12 border-t border-slate-100'>
          ...
          <span className=''>{generations.length} total</span>
        </div>

        <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 '>
          {generations.map((gen) => (
            <div key={gen._id} className='group bg-white rounded-2xl border border-slate-100 p-5 hover:hover-red-200 transition-all relative overflow-hidden'>
      ...
      {activeScheduler && (
        <div className='fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40
          ...
          flex flex-col max-h-[90vh]'>
            ...
            <div className='flex-1 overflow-y-auto p-8 space-y-4'>
              <div className='bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4 '>
                ...
                {activeScheduler.mediaUrl && <img src={activeScheduler.mediaUrl} alt='preview' className='w-full
                aspect-video object-cover rounded-xl border border-slate-200 shadow-sm'/>}
              </div>
            </div>
            ...
            <div className='p-8 bg-slate-50/50 border-t border-slate-50 space-y-8'>
              ...
              <div className='space-y-6'>
                <div>
                  <label className='block text-xs text-slate-600 uppercase tracking-widest mb-4'>Select Channel</label>
