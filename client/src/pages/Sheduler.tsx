      ...
      <div className='w-full lg:w-[460px] shrink-0'>
        <div className='bg-white rounded-2xl border border-slate-200 p-6'>
          ...
          <form className='space-y-5' onSubmit={handleSchedule}>
            ...
            <div>
              <label className='block text-xs text-slate-500 uppercase mb-2' htmlFor="">Plaforms</label>
                ...
                {PLATFORMS.map((p) => {
                  ...
                  return (
                    <button onClick={() => togglePlatform(p.id)}
                      key={p.id} type='button' className={`flex items-center gap-1.5 p-3 rounded-md border transition-all
                    duration-150 ${active ? 'bg-red-50 border-red-300 text-red-500 scale-105' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
            ...
            <div>
              ...
              <textarea required rows={5} placeholder='What do u want to share today?' className='w-full px-5 py-4 bg-slate-50 border
               border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none' value={content}
              ...
              {mediaFile ? (
                <div className='relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50'>
                  {mediaFile.type.startsWith('image/') ?
                    <img src={URL.createObjectURL(mediaFile)} alt='preview' className='w-full h-40
                  object-cover'/> :
                    <video src={URL.createObjectURL(mediaFile)} className='w-full h-40 object-cover' controls />}

                  <button onClick={() => setMediaFile(null)}
                    className='absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-900/80
                ...
                <label className='flex items-center justify-center gap-2 p-5 py-10 border-2 border-dashed border-slate-200 rounded-xl
                cursor-pointer hover:border-red-300 hover:bg-red-50/30 transition-all-group'>
                  <span className='text-sm text-slate-500 group-hover:text-red-600 transitions-color'>Click to upload images or videos</span>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
              <div>
                ...
                <div className='relative'>
                  ...
                  <input type="date" required className='w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 rounded-lg text-slate-900 
              ...
              <div>
                ...
                <div className='relative'>
                  ...
                  <input type="time" required className='w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 rounded-lg text-slate-900 
            ...
            <button type='submit' disabled={loading} className='w-full flex items-center justify-center gap-2 py-3.5 bg-red-500
      ...
      <div className='flex-1 flex flex-col gap-6 min-w-0'>
        ...
        <div className='bg-white rounded-2xl border border-slate-200 overflow-hidden'>
          <div className='flex items-center gap-2.5 px-5 py-4 border-b border-slate-100'>
            ...
            <span className='ml-auto text-xs font-bold'>{scheduled.length}</span>
          </div>
                ...
                <div key={post._id} className='px-5 py-4 hover:bg-slate-50/60 transition-colors'>
                  ...
                  <div className='flex items-center justify-between mb-2'>
                    ...
                    <div className='flex items-center gap-2'>
                      {post.mediaType && <span className='text-xs bg-slate-100 text-slate-600 border border-slate-200
                      px-1.5 py-0.5 rounded-md font-semibold capatalize'>{post.mediaType}</span>}
        ...
        <div className='bg-white rounded-2xl border border-slate-200 overflow-hidden'>
          <div className='flex items-center gap-2.5 px-5 py-4 border-b border-slate-100'>
            ...
            <span className='ml-auto text-xs font-bold'>{published.length}</span>
          </div>
