...
const Sidebar = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (isOpen: boolean) => void }) => {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full
     transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink key={item.name}
              ...
              onClick={() => setIsOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm
      ...
      <div className='p-4 border-t border-slate-100'>
        <div className='flex items-center gap-3 p-2 rounded-xl hover:bg:slate-50 transition-colors'>
          ...
          <div className='size-8 rounded-full bg-linear-to-br from-red-400 to-pink-400 flex items-center justify-center
        ...
        <button onClick={logout} className='mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm text-slate-500
