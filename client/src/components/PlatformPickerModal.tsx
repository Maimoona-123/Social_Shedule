                ...
                {PLATFORMS.map((p) => {
                    ...
                    return(
                        <button key={p.id} disabled={isConnected || isConnecting}
                         onClick={() => onConnect(p.id)} className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all
                         ${isConnected ? 'border-red-200 bg-red-50 cursor-default' : 'border-slate-200 bg-slate-50 hover:border-slate-300 cursor-pointer'}
                         ${isConnecting && 'opacity-60'}`}>
                            ...
                            <div className='flex-1 min-w-0'>
                                <div className={`text-sm ${isConnected ? 'text-red-600' : 'text-slate-900'}`}>
                                    {p.name}
                                </div>
                            ...
                            {isConnected && !isConnecting && <ExternalLinkIcon className='size-3.5 text-slate-500 shrink-0'/>}
                        </button>
                    )
                })}
            </div>
