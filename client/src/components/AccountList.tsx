...
const AccountList = ({accounts, onDisconnect}: AccountListProps) => {
    ...
    const hadleDisconnect = async (accountId: string) => {
        const confirm
         = window.confirm("Are u sure u want to disconnect this account..?")
        if(!confirm) return;
  ...
  return (
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
        {accounts.map((account, index) => {
            ...
            return (
                <div key={index} className='group bg-white border border-slate-200 rounded-2xl p-5 flex items-center
                 gap-4 hover:border-slate-300 transition-all'>
                    ...
                    <button onClick={() => hadleDisconnect(account._id)} title='Disconnect account'
